        ...
        protected override DriverResult Display(RecentlySeenPart part, string displayType, dynamic shapeHelper)
        {
            return ContentShape("Parts_Menu_RecentlySeen",
                () =>
                {
                    ...
                    switch (counterType)
                    {
                        case VisitCounterType.PerSession:
                            ...
                            items = _svc.GetItems(
                                ...
                                .Where(rp => rp.Is<AutoroutePart>())
                                .Select(rp => new RecentlySeenItem
                                {
                                    Url = "/" + rp.As<AutoroutePart>().Path,
                                    Text = new LocalizedString(rp.As<TitlePart>().Title),
                    ...
                    return shapeHelper.Parts_Menu_RecentlySeen(RecentItems: items, ShowCounts: part.ShowCounts);
                });
        }
