...
namespace Szmyd.Orchard.Modules.Menu.Drivers
{
    public class MenuWidgetPartDriver : ContentPartDriver<MenuWidgetPart>
    {
        ...
        protected override DriverResult Display(MenuWidgetPart part, string displayType, dynamic shapeHelper)
        {
            return ContentShape("Parts_Menu_Widget", () =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(part.RootNode))
                {
                    ...
                    if (item != null)
                        items = item.Items.OrderBy(m => m.Position, new PositionComparer()).ToList();
                }
                else
                {
                    switch (part.Mode)
                    {
                        ...
                        case MenuWidgetMode.ChildrenOnly:
                            ...
                            items = item != null ? item.Items.OrderBy(m => m.Position, new PositionComparer()).ToList() : Enumerable.Empty<MenuItem>();
                            break;
                        case MenuWidgetMode.SiblingsOnly:
                            ...
                            if (parent != null)
                                items = parent.Items.OrderBy(m => m.Position, new PositionComparer()).ToList();
                            break;
        ...
        protected override void Exporting(MenuWidgetPart part, ExportContentContext context)
        {
            ...
            context.Element(part.PartDefinition.Name).SetAttributeValue("RootNode", part.RootNode);
            context.Element(part.PartDefinition.Name).SetAttributeValue("WrapChildrenInDiv", part.WrapChildrenInDiv);
            context.Element(part.PartDefinition.Name).SetAttributeValue("Levels", part.Levels);
            context.Element(part.PartDefinition.Name).SetAttributeValue("CutOrFlattenLower", part.CutOrFlattenLower);
        }
        ...
        protected override void Importing(MenuWidgetPart part, ImportContentContext context)
        {
            ...
            part.Mode = DriverImportUtility.GetEnumAttribute<MenuWidgetMode>(context, partName, "Mode");
            part.RootNode = DriverImportUtility.GetAttribute<string>(context, partName, "RootNode");
            part.WrapChildrenInDiv = DriverImportUtility.GetAttribute<bool>(context, partName, "WrapChildrenInDiv");
            part.Levels = DriverImportUtility.GetAttribute<int>(context, partName, "Levels");
            part.CutOrFlattenLower = DriverImportUtility.GetAttribute<bool>(context, partName, "CutOrFlattenLower");
        }
        ...
        internal static MenuItem FindItem(IEnumerable<MenuItem> items, string url)
        {
            ...
            {
                var found = items.FirstOrDefault(i => i.Href.TrimEnd('/').ToUpperInvariant() == url.ToUpperInvariant());
