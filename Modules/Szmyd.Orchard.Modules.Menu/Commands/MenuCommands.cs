...
{
    using global::Orchard.ContentManagement.Aspects;
    ...
    public class MenuCommands : DefaultOrchardCommandHandler
    {
        ...
        [CommandName("menu delete")]
        [CommandHelp("menu delete <menuName>\r\n\t" + "Deletes the menu with the specified name")]
        public void Delete(string menuName) {
            if (String.IsNullOrWhiteSpace(menuName)) {
                throw new OrchardException(T("Menu name was not supplied."));
            }

            var menu = _menuService.GetMenu(menuName);
            if (menu == null) {
                Context.Output.WriteLine(T("Menu {0} was not found. No action taken.", menuName));
            }
            _menuService.DeleteMenu(menuName);
            Context.Output.WriteLine(T("Deleted menu {0}", menuName));
        }
