    ...
    public class MenuAdminController : Controller, IUpdateModel
    {
        ...
        public ActionResult Fill(string menuName, bool containable)
        {
            ...
            var routables = _services
                .ContentManager.Query<AutoroutePart, AutoroutePartRecord>().List().Select(r => r);

            var positions = _nav.BuildMenu(menuName);
            int j = positions.Any() ? int.Parse(PositionUtility.GetNext(positions)) : 1;
